...
function url($file, $old = null, $new = null, $blank = false)
{
...
if (isset($old) and !is_dir($old) and isset($new) and !is_dir($new))
{
  $diff = new diff;
  $text = $diff->inline($old, $new, 2);
    ...
    if (is_dir($old))
      ...
      if (isset($new))
        if (is_dir($new))
          ...
          $hDir = opendir('.'); # Should never happen
      else
        $hDir = opendir(dirname($old));
  else
    $hDir = opendir('.');

  echo "<ul>\n";
  while (($file = readdir($hDir)) !== false) {
    if (!is_dir($file) or $file !== '.')
      echo "<li>" . url($file, $old, $new) . "</li>\n";
  }
  echo "</ul>\n";
